...
import PandaAnalysis.Monojet.Selection_bb as sel
...
if toProcess=='SR0':
  subfolder = 'sr/'
elif toProcess=='WmnCR':
  subfolder = 'cr_w_mu/'
elif toProcess=='WenCR':
  subfolder = 'cr_w_el/'
elif toProcess=='TemCR':
  subfolder = 'cr_ttbar/'
elif toProcess=='ZmmCR':
  subfolder = 'cr_dimuon/'
elif toProcess=='ZeeCR':
  subfolder = 'cr_dielectron/'
#elif toProcess=='pho':
#  subfolder = 'cr_gamma/'
   
forestDir = '/uscms_data/d3/shoh/skim/monojet/'
baseDir = '/uscms_data/d3/shoh/skim/v_8026_0_4/monojet/' + subfolder
lumi = 35900.
...
def shiftBtags(label,tree,varmap,cut,basecut):
  ...
  for shift in ['BUp','BDown','MUp','MDown']:
    #for cent in ['sf_btag','sf_sjbtag','sf_Medbtag']:
    for cent in ['sf_Medbtag']:
      ...
      Weight = sel.weights[basecut+'_'+cent+shift]
...
if enable('test'):
  ...
  cut = sel.cuts['SR0']
  weight = '%f*%s'%(lumi,sel.weights['SR0'])
  processes['test'] = [
...
# wmn
if enable('WmnCR'):
  regions['WmnCR'] = root.Region('WmnCR')
  cut = sel.cuts['WmnCR']
  weight = '%s'%(sel.weights['WmnINC']%lumi)
  PInfo('makeLimitForest.py',cut)
  PInfo('makeLimitForest.py',weight)
  processes['WmnCR'] = [
  ...
  processes['WmnCR'] += shiftBtags('ttbar',tTTbar,vmW,cut,'WmnCR')
  processes['WmnCR'] += shiftBtags('Wlv',tWlv,vmW,cut,'WmnCR')
  processes['WmnCR'] += shiftBtags('ST',tST,vmW,cut,'WmnCR')
  processes['WmnCR'] += shiftBtags('Zll',tZll,vmW,cut,'WmnCR')
  processes['WmnCR'] += shiftBtags('Diboson',tVV,vmW,cut,'WmnCR')
  processes['WmnCR'] += shiftBtags('QCD',tQCD,vmW,cut,'WmnCR')
  for p in processes['WmnCR']:
    regions['WmnCR'].AddProcess(p)
  factory.AddRegion(regions['WmnCR'])
