...
def project(var, cut, weight, samplelist, pd, ntupledir):
    ...
    for i, s in enumerate(samplelist):
        if "HIST" in cut: # Histogram written to file
            ...
            print "samples[",s,"] = ", samples[s]['files']
            for j, ss in enumerate(samples[s]['files']):
...
def drawSignal(hist, sign, log=False):
    ...
    #hist[sign[0]].SetMaximum(max(hist[sign[0]].GetMaximum(), hist[sign[-1]].GetMaximum())*6)
    #hist[sign[0]].SetMinimum(0.)
    hist[sign[0]].SetMinimum(0.1) #WRONG, look for Signal_drawUtils
    if log:
        hist[sign[0]].SetMaximum(max(hist[sign[0]].GetMaximum(), hist[sign[-1]].GetMaximum())*11)
    else:
        hist[sign[0]].SetMaximum(max(hist[sign[0]].GetMaximum(), hist[sign[-1]].GetMaximum())*1.25)
...
def drawCMS(lumi, text,remark, process, cut, onTop=False):
    latex = TLatex()
    latex.SetNDC()
    ...
    latex.DrawLatex(0.95, 0.985, "13 TeV")
...
#    latex.DrawLatex(0.45, 0.98, "DM monojet")
    latex.DrawLatex(0.31, 0.87, remark)
    latex.DrawLatex(0.13, 0.79, process)
    if len(cut) == 4:
        latex.DrawLatex(0.10, 0.96, "Reco-Level")
    else:
        latex.DrawLatex(0.10, 0.96, "Analysis-Level")
